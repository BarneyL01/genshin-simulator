...
import { compareWeapons, customRunInput, runTeam, savedTeamInput, teamInput, type BuildOptions } from '../kb';
import type { CustomSource, Request, Response, RunSettings } from './protocol';
...
const inputOf = (kb: ReturnType<typeof bundledKb>, req: { teamId: string; custom?: CustomSource }) => {
  if (req.custom) return savedTeamInput(kb, req.custom);
  const team = kb.teams.get(req.teamId);
  if (!team) throw new Error(`unknown team "${req.teamId}"`);
  return teamInput(team);
};
...
self.onmessage = (e: MessageEvent<Request>) => {
  ...
  try {
    const kb = bundledKb();
    if (req.type === 'team') {
      reply({ id: req.id, ok: true, type: 'team', result: runTeam(kb, inputOf(kb, req), options(req.settings)) });
      ...
      reply({ id: req.id, ok: true, type: 'compare', result: compareWeapons(kb, inputOf(kb, req), req.characterId, req.candidates, req.baseline, options(req.settings)) });
    }
  } catch (err) {
