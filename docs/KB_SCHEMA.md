...
### Character fields added in M3–M6
...
- `usualCombo[].actions[].action`: a standard action or an `extraActions` key (validated).
- Frames: `frames.cancel` keys are `normal`, `charged`, `skill`, `burst`, `swap`, `dash`, `jump`, `default`. `default` is the animation length (used for any next action not listed and as the action's end).
...
## Effect (fields added in M3–M6 are listed after the block)
...
`members[]`: `{ slot, character, role, substitutes[], weapons[] (best first), artifacts[{ sets }], mainStats? { sands[], goblet[], circlet[] } }` — main stats fall back to the character's `recommended.mainStats`. `rotation`: `{ lengthSeconds?, script[], source, sourceUrl?, original? }` where `script` items are steps or repeat groups (see `docs/SIMULATION.md`, "Rotation scripts"). `sourceRank` records each site's rank separately (empty until Game8/GameWith pages are read).

## Mechanics files added after M3

`kqms.json`: `totalLiquidSubstats`, `individualLiquidCap`, `fixedSubstatCount`, `substatValues`, `mainStatValues` (5★, level 20). `icd.json` is generated by `scripts/kb-import/import-icd.ts`.
